...
removeExtraInfo(struct ins32_t *instruction);
...
pushToPGL(struct ins32_t *instruction);
...
uint8_t disassemble(char *elfFile)
{
	...
	char *args[] = { "/opt/rv32/bin/riscv32-unknown-linux-gnu-objdump",
			 "-d", elfFile, NULL };
...
static uint8_t parseContent(char *assemblyFile)
{
	...
	do {
		...
		if (start) {
			...
			current->disassembled = (char *)calloc(
				(endPos - startPos) + 1, sizeof(char));
			strncpy(current->disassembled, &line[startPos],
				endPos - startPos);
